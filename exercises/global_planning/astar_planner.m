    ...
    function [waypoints, errMsg, cellTypes, heapHead] = run(iterations)
        ...
        if isempty(errorMessage)
                ...
                for i = 1:8
                    if validNeighbours(i)
                        ...
                        if heapPos > 0
                            ...
                            if(g_cost(neighbourCell) > (g_cost(cell)+neighbourCosts(i)))
                                g_cost(neighbourCell) = g_cost(cell)+neighbourCosts(i);
                                [iy,ix] = ind2sub(sz,neighbourCell);
                                h_delta = abs([iy,ix]-[y_goal ,x_goal]);
                                f_cost(neighbourCell) =  g_cost(neighbourCell) + 10 * abs(h_delta(1) - h_delta(2)) + 14 * min(h_delta);
            ...
            if closed(goalCell) % we found a path                
                ...
                cell = goalCell;
                while true
                    [iy,ix] = ind2sub(sz,cell);
                    waypoints = [waypoints, [ix;iy]];
                    cell = parent(cell);
                    if(cell == startCell)break; end
                end
