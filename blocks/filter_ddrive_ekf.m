...
function filter = filter_ddrive_ekf()
    ...
    filter.default_useCartesianSensor = true;      % if true, compute landmark positions (m_ix, m_iy) from range & bearing and use then as output measurements
    ...
    filter.default_useExactDiscretization = true;  % if true: do exact discretization, followed by linearization for covariance propagation
