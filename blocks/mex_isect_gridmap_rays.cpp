...
class IntersectionDetector
{
  public:
	IntersectionDetector(const mxArray *mxMap, const mxArray *mxRayStart, const mxArray *mxRayEnd)
	{
		...
		height = mxGetM(mxMap);
		pitch = height;
		count = mxGetM(mxRayStart); // number fo rays
...
#undef CHECK_CELL
					...
					*pISect++ = isect;
					if (pRange)
					{
						*pRange++ = isect ? sqrt((double)((prevX - xs) * (prevX - xs) + (prevY - ys) * (prevY - ys))) : std::numeric_limits<double>::signaling_NaN();
					}
					continue;
				}
			}
...
{
	if (nrhs < IN_REQ_COUNT)
		mexErrMsgTxt("Too few input arguments");
	if (nrhs > IN_MAX_COUNT)
		mexErrMsgTxt("Too many input arguments");
	if (nlhs < OUT_REQ_COUNT)
		mexErrMsgTxt("Too few output arguments");
	if (nlhs > OUT_MAX_COUNT)
	...
	else if (mxIsLogical(mxMap))
	{
		...
		mexErrMsgTxt("Unsupported data format. This should have been detected earlier!");

	plhs[INDEX_OUT_ISECT] = detector.mxISect().release();
	if (nlhs > INDEX_OUT_RANGE)
		plhs[INDEX_OUT_RANGE] = detector.mxRange().release();
}
